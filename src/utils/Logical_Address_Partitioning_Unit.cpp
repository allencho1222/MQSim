...
std::vector<std::vector<flash_channel_ID_type>>
    Logical_Address_Partitioning_Unit::stream_channel_ids;
std::vector<std::vector<flash_chip_ID_type>>
    Logical_Address_Partitioning_Unit::stream_chip_ids;
std::vector<std::vector<flash_die_ID_type>>
    Logical_Address_Partitioning_Unit::stream_die_ids;
std::vector<std::vector<flash_plane_ID_type>>
    Logical_Address_Partitioning_Unit::stream_plane_ids;
...
void Logical_Address_Partitioning_Unit::Reset() {
  ...
  for (flash_channel_ID_type channel_id = 0; channel_id < channel_count;
       channel_id++) {
    for (flash_chip_ID_type chip_id = 0; chip_id < chip_no_per_channel;
         chip_id++) {
...
void Logical_Address_Partitioning_Unit::Allocate_logical_address_for_flows(
    HostInterface_Types hostinterface_type, unsigned int concurrent_stream_no,
    unsigned int channel_count, unsigned int chip_no_per_channel,
  ...
  for (flash_channel_ID_type channel_id = 0; channel_id < channel_count;
    ...
    for (flash_chip_ID_type chip_id = 0; chip_id < chip_no_per_channel;
      ...
      for (flash_die_ID_type die_id = 0; die_id < die_no_per_chip; die_id++) {
        ...
        for (flash_plane_ID_type plane_id = 0; plane_id < plane_no_per_die;
             plane_id++) {
  ...
  for (unsigned int stream_id = 0; stream_id < concurrent_stream_no;
       stream_id++) {
    for (flash_channel_ID_type channel_id = 0;
         channel_id < stream_channel_ids[stream_id].size(); channel_id++) {
      for (flash_chip_ID_type chip_id = 0;
           chip_id < stream_chip_ids[stream_id].size(); chip_id++) {
        for (flash_die_ID_type die_id = 0;
             die_id < stream_die_ids[stream_id].size(); die_id++) {
          for (flash_plane_ID_type plane_id = 0;
               plane_id < stream_plane_ids[stream_id].size(); plane_id++) {
            if (!(stream_channel_ids[stream_id][channel_id] < channel_count)) {
              PRINT_ERROR("Invalid channel ID specified for I/O flow "
                          << stream_id);
            }

            if (!(stream_chip_ids[stream_id][chip_id] < chip_no_per_channel)) {
              PRINT_ERROR("Invalid chip ID specified for I/O flow "
                          << stream_id);
            }

            if (!(stream_die_ids[stream_id][die_id] < die_no_per_chip)) {
              PRINT_ERROR("Invalid die ID specified for I/O flow "
                          << stream_id);
            }

            if (!(stream_plane_ids[stream_id][plane_id] < plane_no_per_die)) {
              PRINT_ERROR("Invalid plane ID specified for I/O flow "
                          << stream_id);
            }
            resource_list[stream_channel_ids[stream_id][channel_id]]
                         [stream_chip_ids[stream_id][chip_id]]
                         [stream_die_ids[stream_id][die_id]]
                         [stream_plane_ids[stream_id][plane_id]]++;
          }
        }
      }
    }
  ...
  for (unsigned int stream_id = 0; stream_id < concurrent_stream_no;
    ...
    Logical_Address_Partitioning_Unit::stream_channel_ids.push_back(
        channel_ids);
    for (flash_channel_ID_type channel_id = 0;
         channel_id < stream_channel_ids[stream_id].size(); channel_id++) {
      Logical_Address_Partitioning_Unit::stream_channel_ids[stream_id]
          .push_back(stream_channel_ids[stream_id][channel_id]);
    }
    ...
    for (flash_chip_ID_type chip_id = 0;
         chip_id < stream_chip_ids[stream_id].size(); chip_id++) {
      Logical_Address_Partitioning_Unit::stream_chip_ids[stream_id].push_back(
          stream_chip_ids[stream_id][chip_id]);
    }
    ...
    for (flash_die_ID_type die_id = 0;
         die_id < stream_die_ids[stream_id].size(); die_id++) {
      Logical_Address_Partitioning_Unit::stream_die_ids[stream_id].push_back(
          stream_die_ids[stream_id][die_id]);
    }
    ...
    for (flash_plane_ID_type plane_id = 0;
         plane_id < stream_plane_ids[stream_id].size(); plane_id++) {
      Logical_Address_Partitioning_Unit::stream_plane_ids[stream_id].push_back(
          stream_plane_ids[stream_id][plane_id]);
    }
  }
  ...
  for (unsigned int stream_id = 0; stream_id < concurrent_stream_no;
      ...
      for (flash_chip_ID_type chip_id = 0;
        ...
        for (flash_die_ID_type die_id = 0;
          ...
          for (flash_plane_ID_type plane_id = 0;
               plane_id < stream_plane_ids[stream_id].size(); plane_id++) {
            lsa_count += (LHA_type)((
                block_no_per_plane * page_no_per_block * sector_no_per_page *
                (1.0 - overprovisioning_ratio) * 1.0 /
                double(resource_list[stream_channel_ids[stream_id][channel_id]]
                                    [stream_chip_ids[stream_id][chip_id]]
                                    [stream_die_ids[stream_id][die_id]]
                                    [stream_plane_ids[stream_id][plane_id]])));
          }
    ...
    pdas_per_flow.push_back(
        LHA_type(double(lsa_count) / (1.0 - overprovisioning_ratio)));
...
double Logical_Address_Partitioning_Unit::Get_share_of_physcial_pages_in_plane(
...
LHA_type Logical_Address_Partitioning_Unit::Start_lha_available_to_flow(
...
LHA_type Logical_Address_Partitioning_Unit::End_lha_available_to_flow(
    stream_id_type stream_id) {
  if (initialized) {
    switch (hostinterface_type) {
    case HostInterface_Types::SATA:
...
LHA_type
Logical_Address_Partitioning_Unit::LHA_count_allocate_to_flow_from_host_view(
    stream_id_type stream_id) {
  if (initialized) {
    switch (hostinterface_type) {
    case HostInterface_Types::SATA:
      return (end_lhas_per_flow[stream_id] - start_lhas_per_flow[stream_id] +
              1);
      break;
    case HostInterface_Types::NVME:
      return (end_lhas_per_flow[stream_id] - start_lhas_per_flow[stream_id] +
...
LHA_type
Logical_Address_Partitioning_Unit::LHA_count_allocate_to_flow_from_device_view(
    stream_id_type stream_id) {
  if (initialized) {
    switch (hostinterface_type) {
    case HostInterface_Types::SATA: // It is not possible to differentiate
                                    // between streams inside the device when a
                                    // SATA host interface is used, so all lha
                                    // space is available to all streams
      return total_lha_no;
      break;
    case HostInterface_Types::NVME:
      return (end_lhas_per_flow[stream_id] - start_lhas_per_flow[stream_id] +
...
PDA_type Logical_Address_Partitioning_Unit::PDA_count_allocate_to_flow(
    stream_id_type stream_id) {
  if (initialized) {
    switch (hostinterface_type) {
    case HostInterface_Types::SATA: // It is not possible to differentiate
