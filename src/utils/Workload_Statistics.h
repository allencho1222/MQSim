...
// Parameters defined in: B. Van Houdt, "On the necessity of hot and cold data
// identification to reduce the write amplification in flash-based SSDs", Perf.
// Eval., 2014.
struct Workload_Statistics {
  Utils::Workload_Type Type;
  stream_id_type Stream_id;
  double Initial_occupancy_ratio; // Ratio of the logical storage space that is
                                  // fill with data in steady-state
  unsigned int Replay_no;
  unsigned int Total_generated_requests;

  int random_request_type_generator_seed;
  double Read_ratio;
  std::vector<sim_time_type> Write_arrival_time,
      Read_arrival_time; // Histogram with 1us resolution
  ...
  std::vector<unsigned int> Write_size_histogram,
      Read_size_histogram; // Histogram with 1 sector resolution
};
} // namespace Utils
