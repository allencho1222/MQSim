...
void read_configuration_parameters(const string ssd_config_file_path,
  ...
  if (!ssd_config_file) {
    ...
    PRINT_MESSAGE("Writing the default configuration parameters to the "
...
read_workload_definitions(const string workload_defs_file_path) {
  ...
  if (!workload_defs_file) {
    ...
    PRINT_MESSAGE("Writing the default workload definitions to the expected "
    ...
    if (line.compare("USE_INTERNAL_PARAMS") != 0) {
      ...
      if (mqsim_io_scenarios != NULL) {
        ...
        PRINT_MESSAGE("Using MQSim's default workload definitions.");
        PRINT_MESSAGE("Writing the default workload definitions to the "
                      "expected workload definition file.");
        PRINT_MESSAGE("[====================] Done!\n");
      }
    }
  }
  ...
  if (use_default_workloads) {
    ...
    io_flow_1->Channel_IDs[0] = 0;
    io_flow_1->Channel_IDs[1] = 1;
    io_flow_1->Channel_IDs[2] = 2;
    io_flow_1->Channel_IDs[3] = 3;
    io_flow_1->Channel_IDs[4] = 4;
    io_flow_1->Channel_IDs[5] = 5;
    io_flow_1->Channel_IDs[6] = 6;
    io_flow_1->Channel_IDs[7] = 7;
    io_flow_1->Chip_No = 4;
    io_flow_1->Chip_IDs = new flash_chip_ID_type[4];
    io_flow_1->Chip_IDs[0] = 0;
    io_flow_1->Chip_IDs[1] = 1;
    io_flow_1->Chip_IDs[2] = 2;
    io_flow_1->Chip_IDs[3] = 3;
    io_flow_1->Die_No = 2;
    io_flow_1->Die_IDs = new flash_die_ID_type[2];
    io_flow_1->Die_IDs[0] = 0;
    io_flow_1->Die_IDs[1] = 1;
    io_flow_1->Plane_No = 2;
    io_flow_1->Plane_IDs = new flash_plane_ID_type[2];
    io_flow_1->Plane_IDs[0] = 0;
    ...
    io_flow_2->Channel_IDs[0] = 0;
    io_flow_2->Channel_IDs[1] = 1;
    io_flow_2->Channel_IDs[2] = 2;
    io_flow_2->Channel_IDs[3] = 3;
    io_flow_2->Channel_IDs[4] = 4;
    io_flow_2->Channel_IDs[5] = 5;
    io_flow_2->Channel_IDs[6] = 6;
    io_flow_2->Channel_IDs[7] = 7;
    io_flow_2->Chip_No = 4;
    io_flow_2->Chip_IDs = new flash_chip_ID_type[4];
    io_flow_2->Chip_IDs[0] = 0;
    io_flow_2->Chip_IDs[1] = 1;
    io_flow_2->Chip_IDs[2] = 2;
    io_flow_2->Chip_IDs[3] = 3;
    io_flow_2->Die_No = 2;
    io_flow_2->Die_IDs = new flash_die_ID_type[2];
    io_flow_2->Die_IDs[0] = 0;
    io_flow_2->Die_IDs[1] = 1;
    io_flow_2->Plane_No = 2;
    io_flow_2->Plane_IDs = new flash_plane_ID_type[2];
    io_flow_2->Plane_IDs[0] = 0;
  ...
  for (auto io_scen = io_scenarios->begin(); io_scen != io_scenarios->end();
    ...
    SSD_Device ssd(&exec_params->SSD_Device_Configuration,
                   &exec_params->Host_Configuration
                        .IO_Flow_Definitions); // Create SSD_Device based on the
                                               // specified parameters
    exec_params->Host_Configuration.Input_file_path =
        workload_defs_file_path.substr(
            0,
            workload_defs_file_path.find_last_of(
