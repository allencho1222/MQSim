...
sim_time_type Host_Parameter_Set::
...
bool Host_Parameter_Set::Record_Latency = false;
sim_time_type Host_Parameter_Set::ResponseTime_Logging_Period_Length =
    400000; // nanoseconds
std::string Host_Parameter_Set::Input_file_path;
std::vector<std::unique_ptr<IO_Flow_Parameter_Set>> Host_Parameter_Set::IO_Flow_Definitions;

void Host_Parameter_Set::parseYAML(const YAML::Node &hostParams) const {
  try {
    PCIe_Lane_Bandwidth = hostParams["pcie_lane_bandwidth"].as<double>();
    PCIe_Lane_Count = hostParams["pcie_lane_count"].as<unsigned int>();
    ...
    Record_Latency = hostParams["record_latency"].as<bool>();
...
//       } else if (strcmp(param->name(), "ResponseTime_Logging_Period_Length") ==
//                  0) {
//         std::string val = param->value();
