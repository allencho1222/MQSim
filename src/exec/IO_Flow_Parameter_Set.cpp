...
void IO_Flow_Parameter_Set::parseYAML(const YAML::Node &ioParams) {
  try {
    const auto &dataCachingMode =
        ioParams["device_level_data_caching_mode"].as<std::string>();
    if (dataCachingMode == "TURNED_OFF") {
      Device_Level_Data_Caching_Mode = SSD_Components::Caching_Mode::TURNED_OFF;
    } else if (dataCachingMode == "WRITE_CACHE") {
      Device_Level_Data_Caching_Mode =
          SSD_Components::Caching_Mode::WRITE_CACHE;
    } else if (dataCachingMode == "READ_CACHE") {
      Device_Level_Data_Caching_Mode = SSD_Components::Caching_Mode::READ_CACHE;
    } else if (dataCachingMode == "WRITE_READ_CACHE") {
      Device_Level_Data_Caching_Mode =
    ...
    const auto &priorityClass = ioParams["priority_class"].as<std::string>();
    ...
    Channel_IDs =
        ioParams["channel_ids"].as<std::vector<flash_channel_ID_type>>();
    Channel_No = Channel_IDs.size();

    Chip_IDs = ioParams["chip_ids"].as<std::vector<flash_chip_ID_type>>();
    Chip_No = Chip_IDs.size();

    Die_IDs = ioParams["die_ids"].as<std::vector<flash_die_ID_type>>();
    Die_No = Die_IDs.size();

    Plane_IDs = ioParams["plane_ids"].as<std::vector<flash_plane_ID_type>>();
    Plane_No = Plane_IDs.size();

    Initial_Occupancy_Percentage =
        ioParams["initial_occupancy_percentage"].as<unsigned int>();
  } catch (...) {
    PRINT_ERROR("Error in IO_Flow_Parameter_Set_Trace_Based!")
  }
}

void IO_Flow_Parameter_Set_Synthetic::parseYAML(const YAML::Node &ioParams) {
  IO_Flow_Parameter_Set::parseYAML(ioParams);
  try {
    Working_Set_Percentage = ioParams["working_set_percentage"].as<int>();
    // generator type
    Read_Percentage = ioParams["read_percentage"].as<int>();
    // ADDRESS distribution
    Percentage_of_Hot_Region = ioParams["percentage_of_hot_region"].as<int>();
    Generated_Aligned_Addresses =
        ioParams["generated_aligned_addresses"].as<bool>();
    Address_Alignment_Unit = ioParams["address_alignment_unit"].as<int>();
    // Requets size distribution
    Average_Request_Size = ioParams["avg_req_size"].as<int>();
    Variance_Request_Size = ioParams["variance_req_size"].as<int>();
    Seed = ioParams["seed"].as<int>();
    Average_No_of_Reqs_in_Queue = ioParams["avg_num_reqs_in_queue"].as<int>();
    ...
    const auto &addrDist = ioParams["address_distribution"].as<std::string>();
    if (addrDist == "STREAMING") {
      Address_Distribution = Utils::Address_Distribution_Type::STREAMING;
    } else if (addrDist == "RANDOM_HOTCOLD") {
      Address_Distribution = Utils::Address_Distribution_Type::RANDOM_HOTCOLD;
    } else if (addrDist == "RANDOM_UNIFORM") {
    ...
    const auto &reqSizeDist =
        ioParams["request_size_distribution"].as<std::string>();
    if (reqSizeDist == "FIXED") {
      Request_Size_Distribution = Utils::Request_Size_Distribution_Type::FIXED;
    } else if (reqSizeDist == "NORMAL") {
...
void IO_Flow_Parameter_Set_Trace_Based::parseYAML(const YAML::Node &ioParams) {
  IO_Flow_Parameter_Set::parseYAML(ioParams);
  try {
    Relay_Count = ioParams["relay_count"].as<int>();
    Percentage_To_Be_Executed = ioParams["percentage_to_be_executed"].as<int>();
    Preconditioning_File_Paths = 
      ioParams["preconditioning_file_path"].as<std::vector<std::string>>();
