...
std::unordered_map<std::string, Flash_Technology_Type>
    Flash_Parameter_Set::flashTechnologyTypeMap = {
        {"SLC", Flash_Technology_Type::SLC},
        {"MLC", Flash_Technology_Type::MLC},
        {"TLC", Flash_Technology_Type::TLC}};
...
void Flash_Parameter_Set::parseYAML(const YAML::Node &flashParams) const {
  const auto &flashTechnologyType = flashParams["technology"].as<std::string>();
  if (!flashTechnologyTypeMap.contains(flashTechnologyType)) {
    PRINT_ERROR("Unknown flash technology type specified in the input file")
  } else {
    Flash_Technology = flashTechnologyTypeMap[flashTechnologyType];
  }
  const auto &cmdSuspensionMode =
      flashParams["cmd_suspension_mode"].as<std::string>();
  if (!cmdSuspensionModeMap.contains(cmdSuspensionMode)) {
  ...
  Page_Read_Latency_CSB =
      flashParams["page_read_latency_csb"].as<unsigned long long>();
  Page_Read_Latency_MSB =
      flashParams["page_read_latency_msb"].as<unsigned long long>();
  Page_Program_Latency_LSB =
      flashParams["page_program_latency_lsb"].as<unsigned long long>();
  ...
  Suspend_Erase_Time =
      flashParams["suspend_erase_time"].as<unsigned long long>();
  Suspend_Program_Time =
      flashParams["suspend_program_time"].as<unsigned long long>();
  Die_No_Per_Chip = flashParams["num_dies_per_chip"].as<unsigned int>();
  Plane_No_Per_Die = flashParams["num_planes_per_die"].as<unsigned int>();
  Block_No_Per_Plane = flashParams["num_blocks_per_plane"].as<unsigned int>();
  Page_No_Per_Block = flashParams["num_pages_per_block"].as<unsigned int>();
  Page_Capacity = flashParams["page_capacity"].as<unsigned int>();
  Page_Metadat_Capacity =
      flashParams["page_metadata_capacity"].as<unsigned int>();
}
