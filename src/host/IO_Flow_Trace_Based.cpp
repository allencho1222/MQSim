...
Host_IO_Request *IO_Flow_Trace_Based::Generate_next_request() {
  if (current_trace_line.size() == 0 ||
      STAT_generated_request_count >= total_requests_to_be_generated) {
    return NULL;
  }
  ...
  char *pEnd;
  request->LBA_count =
      std::strtoul(current_trace_line[ASCIITraceSizeColumn].c_str(), &pEnd, 0);

  request->Start_LBA = std::strtoull(
      current_trace_line[ASCIITraceAddressColumn].c_str(), &pEnd, 0);
  if (request->Start_LBA <= (end_lsa_on_device - start_lsa_on_device)) {
    request->Start_LBA += start_lsa_on_device;
  } else {
    request->Start_LBA =
        start_lsa_on_device +
        request->Start_LBA % (end_lsa_on_device - start_lsa_on_device);
  }
...
void IO_Flow_Trace_Based::NVMe_consume_io_request(
...
void IO_Flow_Trace_Based::Start_simulation() {
  ...
  while (std::getline(trace_file, trace_line)) {
    ...
    Utils::Helper_Functions::Tokenize(trace_line, ASCIILineDelimiter,
                                      current_trace_line);
    ...
    last_request_arrival_time = std::strtoll(
        current_trace_line[ASCIITraceTimeColumn].c_str(), &pEnd, 10);
...
void IO_Flow_Trace_Based::Execute_simulator_event(MQSimEngine::Sim_Event *) {
  ...
  if (STAT_generated_request_count < total_requests_to_be_generated) {
    ...
    if (std::getline(trace_file, trace_line)) {
      ...
      Utils::Helper_Functions::Tokenize(trace_line, ASCIILineDelimiter,
                                        current_trace_line);
      PRINT_MESSAGE("* Replay round " << replay_counter << "of "
                                      << total_replay_no << " started  for"
                                      << ID())
    }
    char *pEnd;
    Simulator->Register_sim_event(
...
void IO_Flow_Trace_Based::Get_statistics(
  ...
  while (std::getline(trace_file_temp, trace_line)) {
    ...
    sim_time_type prev_time = last_request_arrival_time;
    last_request_arrival_time =
        std::strtoull(line_splitted[ASCIITraceTimeColumn].c_str(), &pEnd, 10);
    if (last_request_arrival_time < prev_time) {
      PRINT_ERROR("Unexpected request arrival time: "
                  << last_request_arrival_time
    ...
    sim_time_type diff =
        ...
        1000; // The arrival rate histogram is stored in the microsecond unit
    ...
    LHA_type start_LBA =
        std::strtoull(line_splitted[ASCIITraceAddressColumn].c_str(), &pEnd, 0);
    ...
    while (start_LBA <= end_LBA) {
      ...
      if (line_splitted[ASCIITraceTypeColumn].compare(ASCIITraceWriteCode) ==
          0) {
