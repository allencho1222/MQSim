...
PCIe_Root_Complex::PCIe_Root_Complex(
    PCIe_Link *pcie_link, HostInterface_Types SSD_device_type,
    SATA_HBA *sata_hba, std::vector<Host_Components::IO_Flow_Base *> *IO_flows)
    : pcie_link(pcie_link), SSD_device_type(SSD_device_type),
      sata_hba(sata_hba), IO_flows(IO_flows) {}

void PCIe_Root_Complex::Write_to_memory(const uint64_t address,
                                        const void *payload) {
  // This is a request to write back a read request data into memory (in modern
  // systems the write is done to LLC)
  if (address >= DATA_MEMORY_REGION) {
    ...
    case HostInterface_Types::NVME: {
      unsigned int flow_id =
          QUEUE_ID_TO_FLOW_ID(((Completion_Queue_Entry *)payload)->SQ_ID);
      ((*IO_flows)[flow_id])
          ->NVMe_consume_io_request((Completion_Queue_Entry *)payload);
      break;
    }
...
void PCIe_Root_Complex::Write_to_device(uint64_t address,
...
void PCIe_Root_Complex::Read_from_memory(const uint64_t address,
  ...
  new_pcie_message->Destination =
      Host_Components::PCIe_Destination_Type::DEVICE;
  ...
  if (address >= DATA_MEMORY_REGION) {
    ...
    new_pcie_message->Payload =
        NULL; // No need to transfer data in the standalone mode of MQSim
  } else {
    switch (SSD_device_type) {
    case HostInterface_Types::NVME: {
      uint16_t flow_id =
...
void PCIe_Root_Complex::Set_io_flows(
