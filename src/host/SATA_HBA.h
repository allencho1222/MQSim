...
public:
  SATA_HBA(sim_object_id_type id, uint16_t ncq_size,
...
private:
  uint16_t ncq_size;
  sim_time_type hba_processing_delay; // This estimates the processing delay of
                                      // the whole SATA software and hardware
                                      // layers to send/receive a SATA message
  PCIe_Root_Complex *pcie_root_complex;
  std::vector<Host_Components::IO_Flow_Base *> *IO_flows;
  ...
  std::list<Host_IO_Request *>
      waiting_requests_for_submission; // The I/O requests that are still
                                       // waiting (since the I/O queue is full)
