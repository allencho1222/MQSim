...
IO_Flow_Synthetic::IO_Flow_Synthetic(
    ...
    double working_set_ratio, uint16_t io_queue_id,
    uint16_t nvme_submission_queue_size, uint16_t nvme_completion_queue_size,
    IO_Flow_Priority_Class::Priority priority_class, double read_ratio,
    ...
    unsigned int average_request_size, unsigned int variance_request_size,
    ...
    bool generate_aligned_addresses, unsigned int alignment_value, int seed,
    sim_time_type stop_time, double initial_occupancy_ratio,
    ...
    : IO_Flow_Base(name, flow_id, start_lsa_on_device,
      ...
      variance_request_size(variance_request_size),
      generator_type(generator_type),
      Average_inter_arrival_time_nano_sec(Average_inter_arrival_time_nano_sec),
  ...
  random_request_type_generator =
      new Utils::RandomGenerator(random_request_type_generator_seed);
  random_address_generator_seed = seed++;
  random_address_generator =
      new Utils::RandomGenerator(random_address_generator_seed);
  ...
  if (address_distribution ==
      Utils::Address_Distribution_Type::RANDOM_HOTCOLD) {
    random_hot_address_generator_seed = seed++;
    random_hot_address_generator =
        new Utils::RandomGenerator(random_hot_address_generator_seed);
    random_hot_cold_generator_seed = seed++;
    random_hot_cold_generator =
        new Utils::RandomGenerator(random_hot_cold_generator_seed);
    hot_region_end_lsa = this->start_lsa_on_device +
                         (LHA_type)((double)(this->end_lsa_on_device -
                                             this->start_lsa_on_device) *
                                    hot_region_ratio);
  }

  if (request_size_distribution ==
      Utils::Request_Size_Distribution_Type::NORMAL) {
    random_request_size_generator_seed = seed++;
    random_request_size_generator =
        new Utils::RandomGenerator(random_request_size_generator_seed);
  }

  if (generator_type == Utils::Request_Generator_Type::BANDWIDTH) {
    random_time_interval_generator_seed = seed++;
    random_time_interval_generator =
        new Utils::RandomGenerator(random_time_interval_generator_seed);
  }
...
Host_IO_Request *IO_Flow_Synthetic::Generate_next_request() {
  ...
  case Utils::Request_Size_Distribution_Type::NORMAL: {
    double temp_request_size = random_request_size_generator->Normal(
        average_request_size, variance_request_size);
    ...
    if (generate_aligned_addresses) {
      if (streaming_next_address % alignment_value != 0) {
        streaming_next_address +=
            alignment_value - (streaming_next_address % alignment_value);
      }
    }
    if (streaming_next_address == request->Start_LBA) {
      PRINT_MESSAGE("Synthetic Message Generator: The same address is always "
                    "repeated due to configuration parameters!")
    }
    break;
  case Utils::Address_Distribution_Type::RANDOM_HOTCOLD:
    ...
    if (random_hot_cold_generator->Uniform(0, 1) < hot_region_ratio) {
      request->Start_LBA = random_hot_address_generator->Uniform_ulong(
          hot_region_end_lsa + 1, end_lsa_on_device);
      if (request->Start_LBA < hot_region_end_lsa + 1 ||
          request->Start_LBA > end_lsa_on_device) {
      ...
      request->Start_LBA = random_hot_address_generator->Uniform_ulong(
          start_lsa_on_device, hot_region_end_lsa);
      if (request->Start_LBA < start_lsa_on_device ||
          request->Start_LBA > hot_region_end_lsa) {
        PRINT_ERROR("Out of range address is generated in IO_Flow_Synthetic!\n")
      }
    }
    break;
  case Utils::Address_Distribution_Type::RANDOM_UNIFORM:
    request->Start_LBA = random_address_generator->Uniform_ulong(
...
void IO_Flow_Synthetic::NVMe_consume_io_request(
    Completion_Queue_Entry *io_request) {
  ...
  if (generator_type == Utils::Request_Generator_Type::QUEUE_DEPTH) {
     ...
     * may return NULL if 1) the simulation stop is met, or 2) the number of
...
void IO_Flow_Synthetic::Start_simulation(bool isPreconditioning) {
  ...
  if (generator_type == Utils::Request_Generator_Type::BANDWIDTH) {
    Simulator->Register_sim_event(
        (sim_time_type)random_time_interval_generator->Exponential(
            (double)Average_inter_arrival_time_nano_sec),
        this, 0, 0);
  } else {
...
void IO_Flow_Synthetic::Get_statistics(
  ...
  stats.random_time_interval_generator_seed =
      random_time_interval_generator_seed;
  stats.Average_inter_arrival_time_nano_sec =
      Average_inter_arrival_time_nano_sec;
