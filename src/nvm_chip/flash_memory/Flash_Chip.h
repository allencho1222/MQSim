...
public:
  Flash_Chip(const sim_object_id_type &, flash_channel_ID_type channelID,
  ...
  void EndCMDDataInXfer(
      Flash_Command *command) // End transferring write data of a group of
                              // multi-plane transactions to the Flash chip
  {
    this->STAT_totalXferTime += (Simulator->Time() - this->lastTransferStart);
    if (this->idleDieNo != die_no)
      STAT_totalOverlappedXferExecTime +=
  ...
  typedef void (*ChipReadySignalHandlerType)(Flash_Chip *targetChip,
                                             Flash_Command *command);
  void Connect_to_chip_ready_signal(ChipReadySignalHandlerType);

  sim_time_type Get_command_execution_latency(command_code_type CMDCode,
    ...
    if (flash_technology == Flash_Technology_Type::MLC) {
      ...
      // From: Yaakobi et al., "Characterization and Error-Correcting Codes for
      // TLC Flash Memories", ICNC 2012
      latencyType =
