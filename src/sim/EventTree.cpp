...
void EventTree::RestoreAfterInsert(EventTreeNode *x) {
  ...
  while (x != rbTree && x->Parent->Color == 0) {
    ...
    if (x->Parent == x->Parent->Parent->Left) {
      ...
      if (y != NULL &&
          y->Color == 0) { // uncle is red; change x's Parent and uncle to black
        x->Parent->Color = 1;
        y->Color = 1;
...
void EventTree::RotateLeft(EventTreeNode *x) {
  // pushing node x down and to the Left to balance the tree. x's Right child
  // (y) replaces x (since y > x), and y's Left child becomes x's Right child
  // (since it's < y but > x).
...
void EventTree::RotateRight(EventTreeNode *x) {
  // pushing node x down and to the Right to balance the tree. x's Left child
  // (y) replaces x (since x < y), and y's Right child becomes x's Left child
  // (since it's < x but > y).
...
///< summary>
sim_time_type EventTree::Get_min_key() { return Get_min_node()->Key; }

///< summary>
/// Get_min_value
/// Returns the object having the minimum key value
///< summary>
Sim_Event *EventTree::Get_min_value() { return Get_min_node()->FirstSimEvent; }
...
///< summary>
void EventTree::Remove_min() { Remove(Get_min_key()); }
