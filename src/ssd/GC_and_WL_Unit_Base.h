...
enum class GC_Block_Selection_Policy_Type {
  GREEDY,
  RGA, /*The randomized-greedy algorithm described in: "B. Van Houdt, A Mean
          Field Model for a Class of Garbage Collection Algorithms in Flash -
          based Solid State Drives, SIGMETRICS, 2013" and "Stochastic Modeling
          of Large-Scale Solid-State Storage Systems: Analysis, Design Tradeoffs
          and Optimization, SIGMETRICS, 2013".*/
  RANDOM,
  RANDOM_P,
  RANDOM_PP, /*The RANDOM, RANDOM+, and RANDOM++ algorithms described in: "B.
                Van Houdt, A Mean Field Model  for a Class of Garbage Collection
                Algorithms in Flash - based Solid State Drives, SIGMETRICS,
                2013".*/
  FIFO /*The FIFO algortihm described in P. Desnoyers, "Analytic  Modeling  of
          SSD Write Performance, SYSTOR, 2012".*/
};
...
public:
  GC_and_WL_Unit_Base(
      ...
      double gc_threshold, bool preemptible_gc_enabled,
      double gc_hard_threshold, unsigned int channel_count,
  ...
  virtual void Check_gc_required(
      ...
      const NVM::FlashMemory::Physical_Page_Address &planeAddress) = 0;
  GC_Block_Selection_Policy_Type Get_gc_policy();
  unsigned int
  Get_GC_policy_specific_parameter(); // Returns the parameter specific to the
                                      // GC block selection policy: threshold
                                      // for random_pp, set_size for RGA
...
protected:
  ...
  NVM_PHY_ONFI *flash_controller;
  bool force_gc;
  double gc_threshold; // As the ratio of free pages to the total number of
                       // physical pages
  ...
  bool is_safe_gc_wl_candidate(
      const PlaneBookKeepingType *pbke,
      const flash_block_ID_type
          gc_wl_candidate_block_id); // Checks if block_address is a safe
                                     // candidate for gc execution, i.e., 1) it
                                     // is not a write frontier, and 2) there is
                                     // no ongoing program operation
  bool check_static_wl_required(
      const NVM::FlashMemory::Physical_Page_Address plane_address);
  void run_static_wearleveling(
      const NVM::FlashMemory::Physical_Page_Address plane_address);
  bool use_copyback;
  ...
  // Used to implement: "Preemptible I/O Scheduling of Garbage Collection for
  // Solid State Drives", TCAD 2013.
  bool preemptible_gc_enabled;
  double gc_hard_threshold;
  unsigned int block_pool_gc_hard_threshold;
  unsigned int
      max_ongoing_gc_reqs_per_plane; // This value has two important usages: 1)
                                     // maximum number of concurrent gc
                                     // operations per plane, and 2) the value
                                     // that determines urgent GC execution when
                                     // there is a shortage of flash blocks. If
                                     // the block bool size drops below this
                                     // value, all incomming user writes should
                                     // be blocked

  // Following variabels are used based on the type of GC block selection policy
  unsigned int rga_set_size; // The number of random flash blocks that are
                             // radnomly selected
