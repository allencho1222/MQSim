...
public:
  NVM::FlashMemory::Flash_Command
      *ActiveCommand; // The current command that is executing on the die

  /*The current transactions that are being serviced. For the set of
  transactions in ActiveTransactions, there is one ActiveCommand that is geting
  executed on the die. Transaction is a FTL-level concept, and command is a
  flash chip-level concept*/
  ...
  void PrepareResume() {
    ...
    ActiveTransactions.insert(ActiveTransactions.begin(),
                              SuspendedTransactions.begin(),
                              SuspendedTransactions.end());
    Suspended = false;
...
public:
  ...
  void Change_flash_page_status_for_preconditioning(
  ...
  Is_chip_busy_with_stream(NVM_Transaction_Flash *transaction);
...
private:
  ...
  void
  perform_interleaved_cmd_data_transfer(NVM::FlashMemory::Flash_Chip *chip,
  ...
  static void
  handle_ready_signal_from_chip(NVM::FlashMemory::Flash_Chip *chip,
