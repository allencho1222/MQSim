...
Data_Cache_Manager_Flash_Simple::Data_Cache_Manager_Flash_Simple(
    ...
    : Data_Cache_Manager_Base(id, host_interface, firmware, dram_row_size,
                              ...
                              Cache_Sharing_Mode::SHARED, stream_count),
      flash_controller(flash_controller),
      capacity_in_bytes(total_capacity_in_bytes),
      sector_no_per_page(sector_no_per_page), request_queue_turn(0),
...
void Data_Cache_Manager_Flash_Simple::process_new_user_request(
  ...
  if (user_request->Type == UserRequestType::READ) {
      ...
      while (it != user_request->Transaction_list.end()) {
        ...
        if (data_cache->Exists(tr->Stream_id, tr->LPA)) {
          page_status_type available_sectors_bitmap =
              data_cache->Get_slot(tr->Stream_id, tr->LPA)
    ...
    default:
      PRINT_ERROR("The specified caching mode is not not support in simple "
                  "cache manager!")
    }
                                    ...
                                    // a destage buffer
    {
      ...
      if (user_request->Transaction_list.size() > 0) {
        waiting_user_requests_queue_for_dram_free_slot[user_request->Stream_id]
            .push_back(user_request);
      }
    }
    default:
      PRINT_ERROR("The specified caching mode is not not support in simple "
...
void Data_Cache_Manager_Flash_Simple::write_to_destage_buffer(
    User_Request *user_request) {
  ...
  while (it != user_request->Transaction_list.end() &&
    ...
    if (data_cache->Exists(
            tr->Stream_id,
            tr->LPA)) // If the logical address already exists in the cache
    {
      ...
      data_cache->Insert_write_data(tr->Stream_id, tr->LPA, tr->Content,
                                    tr->DataTimeStamp,
                                    tr->write_sectors_bitmap);
    }
    ...
    if (bloom_filter[0].find(tr->LPA) == bloom_filter[0].end()) {
      data_cache->Change_slot_status_to_writeback(
...
void Data_Cache_Manager_Flash_Simple::
    handle_transaction_serviced_signal_from_PHY(
  ...
  if (transaction->Type == Transaction_Type::READ) {
    ...
    switch (Data_Cache_Manager_Flash_Simple::caching_mode_per_input_stream
    ...
    case Caching_Mode::TURNED_OFF:
      ...
      break;
    case Caching_Mode::WRITE_CACHE: {
      ((Data_Cache_Manager_Flash_Simple *)_my_instance)
          ->back_pressure_buffer_depth -=
      ...
      if (((Data_Cache_Manager_Flash_Simple *)_my_instance)
              ->data_cache->Exists(
                  transaction->Stream_id,
      ...
      for (unsigned int i = 0; i < _my_instance->stream_count; i++) {
        ...
        ((Data_Cache_Manager_Flash_Simple *)_my_instance)->request_queue_turn %=
            ((Data_Cache_Manager_Flash_Simple *)_my_instance)->stream_count;
        if (((Data_Cache_Manager_Flash_Simple *)_my_instance)
                ->waiting_user_requests_queue_for_dram_free_slot
                    [((Data_Cache_Manager_Flash_Simple *)_my_instance)
                         ->request_queue_turn]
...
void Data_Cache_Manager_Flash_Simple::service_dram_access_request(
...
void Data_Cache_Manager_Flash_Simple::Execute_simulator_event(
  ...
  case Data_Cache_Simulation_Event_Type::MEMORY_WRITE_FOR_USERIO_FINISHED:
    ((User_Request *)(transfer_inf)->Related_request)
        ->Sectors_serviced_from_cache -=
        transfer_inf->Size_in_bytes / SECTOR_SIZE_IN_BYTE;
  ...
  if (dram_execution_queue[transfer_inf->Stream_id].size() > 0) {
    Memory_Transfer_Info *new_transfer_info =
        dram_execution_queue[transfer_inf->Stream_id].front();
    Simulator->Register_sim_event(
        Simulator->Time() + estimate_dram_access_time(
                                new_transfer_info->Size_in_bytes, dram_row_size,
                                dram_busrt_size, dram_burst_transfer_time_ddr,
                                dram_tRCD, dram_tCL, dram_tRP),
        this, new_transfer_info,
        static_cast<int>(new_transfer_info->next_event_type));
  }
