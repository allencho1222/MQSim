...
typedef uint64_t
    LHA_type; // Logical Host Address, the addresses unit on the host-side. As
              // of 2018, LHA is mainly a sector (i.e., 512B) but it could be as
              // small as a cache-line (i.e., 64B) in future NVMs.
typedef uint64_t PDA_type; // Physical device address, could be a 1) sector, 2)
                           // subpage, or a 3) cacheline
...
inline unsigned int
count_sector_no_from_status_bitmap(const page_status_type page_status) {
  unsigned int size = 0;
  for (int i = 0; i < 64; i++) {
