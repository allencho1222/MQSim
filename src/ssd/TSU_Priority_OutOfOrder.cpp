...
void TSU_Priority_OutOfOrder::initQueue() {
  ...
  _NVMController->SetTRQueue(
    UserReadTRQueue, 
    UserWriteTRQueue, 
    GCReadTRQueue, 
    GCWriteTRQueue,
    GCShallowEraseTRQueue,
    GCFullEraseTRQueue,
    MappingReadTRQueue, 
    MappingWriteTRQueue);
}
...
// void TSU_Priority_OutOfOrder::releaseWaitingRequests(LPA_type lpa) {
// }

void TSU_Priority_OutOfOrder::Schedule(bool fromGC) {
    ...
    case Transaction_Type::READ:
      ...
      case Transaction_Source_Type::USERIO:
        if ((*it)->Priority_class != IO_Flow_Priority_Class::UNDEFINED) {
          // if (!pendingLPAs.contains((*it)->LPA)) {
          bool inserted = false;
          UserReadTRQueue[(*it)->Address.ChannelID][(*it)->Address.ChipID]
                         [static_cast<int>((*it)->Priority_class)]
                             .push_back((*it));
          // pendingLPAs[(*it)->LPA] = std::vector<NVM_TransactioN_Flash*>();
          // } else {
          //   pendingLPAs.push_back(*it);
          // }

          //   auto* q = &UserReadTRQueue[(*it)->Address.ChannelID][(*it)->Address.ChipID]
          //                  [static_cast<int>((*it)->Priority_class)];
          //   // if (lockedQueue.contains[(*it)->LPA]) {
          //   //   lockedQueue.erase((*it)->LPA);
          //   // }
          //   lockedQueue[(*it)->LPA].push_back(std::make_pair(q, --std::end(*q)));
          // }
  ...
  for (flash_channel_ID_type channelID = 0; channelID < channel_count;
    ...
    if (_NVMController->Get_channel_status(channelID) ==
      ...
      for (unsigned int i = 0; i < chip_no_per_channel; i++) {
        ...
        if (!service_aero_read(chip, fromGC)) {
          // if (!service_write_transaction(chip)) {
          if (!service_aero_write(chip, fromGC)) {
...
bool TSU_Priority_OutOfOrder::service_aero_read(
    NVM::FlashMemory::Flash_Chip *chip, bool fromGC) {
