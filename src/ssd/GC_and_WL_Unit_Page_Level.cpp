...
GC_and_WL_Unit_Page_Level::GC_and_WL_Unit_Page_Level(
    ...
    : GC_and_WL_Unit_Base(
          ...
          gc_hard_threshold, ChannelCount, chip_no_per_channel, die_no_per_chip,
...
bool GC_and_WL_Unit_Page_Level::GC_is_in_urgent_mode(
    const NVM::FlashMemory::Flash_Chip *chip) {
  if (!preemptible_gc_enabled) {
    return true;
  }

  NVM::FlashMemory::Physical_Page_Address addr;
  addr.ChannelID = chip->ChannelID;
  addr.ChipID = chip->ChipID;
...
void GC_and_WL_Unit_Page_Level::Check_gc_required(
  ...
  if (free_block_pool_size < block_pool_gc_threshold) {
    ...
    case SSD_Components::GC_Block_Selection_Policy_Type::RGA: {
      ...
      while (random_set.size() < rga_set_size) {
        flash_block_ID_type block_id =
            random_generator.Uniform_uint(0, block_no_per_plane - 1);
        if (pbke->Ongoing_erase_operations.find(block_id) ==
                pbke->Ongoing_erase_operations.end() &&
            is_safe_gc_wl_candidate(pbke, block_id)) {
    ...
    case SSD_Components::GC_Block_Selection_Policy_Type::RANDOM: {
      ...
      while (!is_safe_gc_wl_candidate(pbke, gc_candidate_block_id) &&
    ...
    case SSD_Components::GC_Block_Selection_Policy_Type::RANDOM_P: {
      ...
      // A write frontier block or a block with free pages should not be
      // selected for garbage collection
      while ((pbke->Blocks[gc_candidate_block_id].Current_page_write_index <
                  pages_no_per_block ||
          ...
          if (block_manager->Is_page_valid(block, pageID)) {
            ...
            if (use_copyback) {
              ...
              gc_write = new NVM_Transaction_Flash_WR(
                  ...
                  gc_candidate_address, NULL, 0, gc_read, 0,
                  INVALID_TIME_STAMP);
              ...
              tsu->Submit_transaction(
                  gc_read); // Only the read transaction would be submitted. The
