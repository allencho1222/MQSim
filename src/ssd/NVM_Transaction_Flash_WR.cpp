...
NVM_Transaction_Flash_WR::NVM_Transaction_Flash_WR(
    ...
    bool is_from_cache)
    : NVM_Transaction_Flash(source, Transaction_Type::WRITE, stream_id,
                            data_size_in_byte, lpa, ppa, address,
                            user_io_request, IO_Flow_Priority_Class::UNDEFINED),
      Content(content), RelatedRead(related_read),
...
NVM_Transaction_Flash_WR::NVM_Transaction_Flash_WR(
    ...
    bool is_from_cache)
    : NVM_Transaction_Flash(source, Transaction_Type::WRITE, stream_id,
                            data_size_in_byte, lpa, ppa, user_io_request,
...
NVM_Transaction_Flash_WR::NVM_Transaction_Flash_WR(
    ...
    NVM::memory_content_type content, page_status_type written_sectors_bitmap,
    data_timestamp_type data_timestamp, bool is_from_cache)
    : NVM_Transaction_Flash(source, Transaction_Type::WRITE, stream_id,
                            data_size_in_byte, lpa, NO_PPA, user_io_request,
                            priority_class),
      Content(content), RelatedRead(NULL),
      write_sectors_bitmap(written_sectors_bitmap),
      DataTimeStamp(data_timestamp),
      ExecutionMode(WriteExecutionModeType::SIMPLE) {
    this->is_from_cache = is_from_cache;
