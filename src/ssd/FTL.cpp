...
FTL::FTL(const sim_object_id_type &id,
         Data_Cache_Manager_Base *data_cache_manager, unsigned int channel_no,
         ...
         unsigned int page_no_per_block, unsigned int page_size_in_sectors,
         sim_time_type avg_flash_read_latency,
         sim_time_type avg_flash_program_latency,
         double over_provisioning_ratio,
         unsigned int max_allowed_block_erase_count, int seed)
    : NVM_Firmware(id, data_cache_manager), random_generator(seed),
      channel_no(channel_no), chip_no_per_channel(chip_no_per_channel),
      ...
      page_size_in_sectors(page_size_in_sectors),
      avg_flash_read_latency(avg_flash_read_latency),
      avg_flash_program_latency(avg_flash_program_latency),
      over_provisioning_ratio(over_provisioning_ratio),
      max_allowed_block_erase_count(max_allowed_block_erase_count) {
  Stats::Init_stats(channel_no, chip_no_per_channel, die_no_per_chip,
                    plane_no_per_die, block_no_per_plane, page_no_per_block,
                    max_allowed_block_erase_count);
}

FTL::~FTL() {
  Stats::Clear_stats(channel_no, chip_no_per_channel, die_no_per_chip,
                     plane_no_per_die, block_no_per_plane, page_no_per_block,
                     max_allowed_block_erase_count);
}
...
void FTL::Perform_precondition(
  ...
  for (auto const &stat : workload_stats) {
    if (stat->Type == Utils::Workload_Type::SYNTHETIC) {
      ...
      case Utils::Request_Generator_Type::BANDWIDTH:
        overall_rate +=
            1.0 / double(stat->Average_inter_arrival_time_nano_sec) *
            SIM_TIME_TO_SECONDS_COEFF * stat->Average_request_size_sector;
        break;
      case Utils::Request_Generator_Type::QUEUE_DEPTH: {
        sim_time_type max_arrival_time = sim_time_type(
            stat->Read_ratio * double(avg_flash_read_latency) +
            (1 - stat->Read_ratio) * double(avg_flash_program_latency));
        double avg_arrival_time =
            double(max_arrival_time) / double(stat->Request_queue_depth);
        overall_rate += 1.0 / avg_arrival_time * SIM_TIME_TO_SECONDS_COEFF *
                        stat->Average_request_size_sector;
        break;
      }
      default:
        PRINT_ERROR("Unknown request type generator in the FTL preconditioning "
                    "function.")
      }
    } else {
      overall_rate += 1.0 / double(stat->Average_inter_arrival_time_nano_sec) *
  ...
  for (auto &stat : workload_stats) {
    LPA_type no_of_logical_pages_in_steadystate =
        (LPA_type)(stat->Initial_occupancy_ratio *
                   Address_Mapping_Unit->Get_logical_pages_count(
                       stat->Stream_id));
    ...
    std::map<LPA_type, page_status_type>
        lpa_set_for_preconditioning; // Stores the accessed LPAs
    std::multimap<int, LPA_type, std::greater<int>>
        trace_lpas_sorted_histogram; // only used for trace workloads
    unsigned int hot_region_last_index_in_histogram =
        0; // only used for trace workloads to detect hot addresses
    ...
    LPA_type max_lpa =
        Convert_host_logical_address_to_device_address(max_lha) - 1;
    total_accessed_cmt_entries +=
        (unsigned int)(Convert_host_logical_address_to_device_address(max_lha) /
                           page_size_in_sectors -
                       Convert_host_logical_address_to_device_address(min_lha) /
                           page_size_in_sectors) +
        1;
    bool hot_range_finished =
        false; // Used for fast address generation in hot/cold traffic mode
    LHA_type hot_region_end_lsa = 0,
             hot_lha_used_for_generation =
                 0; // Used for fast address generation in hot/cold traffic mode
    ...
    if (stat->Type == Utils::Workload_Type::SYNTHETIC) {
      ...
      if (stat->Address_distribution_type ==
          Utils::Address_Distribution_Type::
              RANDOM_HOTCOLD) // treat a workload with very low hot/cold values
      ...
      case Utils::Address_Distribution_Type::RANDOM_HOTCOLD: {
        random_hot_address_generator =
            new Utils::RandomGenerator(stat->random_hot_address_generator_seed);
        random_hot_cold_generator =
            new Utils::RandomGenerator(stat->random_hot_cold_generator_seed);
        hot_region_end_lsa =
            min_lha +
            (LHA_type)((double)(max_lha - min_lha) *
                       stat->Ratio_of_hot_addresses_to_whole_working_set);
        last_hot_lpa =
            Convert_host_logical_address_to_device_address(hot_region_end_lsa) -
            1; // Be conservative and not include the last_hot_address itself
        hot_lha_used_for_generation = min_lha;
        // Check if enough LPAs could be generated within the working set of the
        // flow
        if ((last_hot_lpa - min_lpa) < no_of_logical_pages_in_steadystate)
          fully_include_hot_addresses = true;
        if ((max_lpa - last_hot_lpa) <
            1.1 * (no_of_logical_pages_in_steadystate -
                   (last_hot_lpa - min_lpa))) {
          PRINT_MESSAGE("The specified initial occupancy value could not be "
                        "satisfied as the working set of workload #"
                        << stat->Stream_id
                        << " is small. MQSim made some adjustments!");
          max_lha = min_lha + LHA_type(double(max_lha - min_lha) /
                                       stat->Working_set_ratio);
          if (stat->generate_aligned_addresses)
            if (max_lha % stat->alignment_value != 0)
              max_lha -= min_lha % stat->alignment_value;
      ...
      case Utils::Address_Distribution_Type::STREAMING: {
        streaming_next_address =
            random_address_generator->Uniform_ulong(min_lha, max_lha);
      ...
      case Utils::Address_Distribution_Type::RANDOM_UNIFORM: {
        ...
        if ((max_lpa - min_lpa) < 1.1 * no_of_logical_pages_in_steadystate) {
          ...
          if ((max_lpa - min_lpa) < 1.1 * no_of_logical_pages_in_steadystate) {
            no_of_logical_pages_in_steadystate =
                (unsigned int)(double(max_lpa - min_lpa) * 0.9);
          }
        }
        break;
      }
      ...
      if (stat->Request_size_distribution_type ==
          Utils::Request_Size_Distribution_Type::NORMAL) {
        random_request_size_generator = new Utils::RandomGenerator(
            stat->random_request_size_generator_seed);
      }

      while (lpa_set_for_preconditioning.size() <
             no_of_logical_pages_in_steadystate) {
        ...
        case Utils::Request_Size_Distribution_Type::NORMAL: {
          double temp_request_size = random_request_size_generator->Normal(
              stat->Average_request_size_sector, stat->STDEV_reuqest_size);
        ...
        case Utils::Address_Distribution_Type::RANDOM_HOTCOLD: {
          if (fully_include_hot_addresses) // just to speedup address generation
          {
            if (!hot_range_finished) {
              ...
              start_LBA = random_hot_address_generator->Uniform_ulong(
                  hot_region_end_lsa + 1, max_lha);
              if (start_LBA < hot_region_end_lsa + 1 || start_LBA > max_lha)
                PRINT_ERROR(
                    "Out of range address is generated in IO_Flow_Synthetic!\n")
              if (start_LBA + size > max_lha)
        ...
        LPA_type max_lpa_within_device =
            Convert_host_logical_address_to_device_address(stat->Max_LHA) -
            Convert_host_logical_address_to_device_address(stat->Min_LHA);
        while (handled_sectors_count < size) {
          transaction_size =
              page_size_in_sectors - (unsigned int)(lsa % page_size_in_sectors);
          ...
          page_status_type access_status_bitmap =
              Find_NVM_subunit_access_bitmap(lsa);

          lsa = lsa + transaction_size;
          handled_sectors_count += transaction_size;

          if (lpa_set_for_preconditioning.find(lpa) ==
              lpa_set_for_preconditioning.end()) {
            lpa_set_for_preconditioning[lpa] = access_status_bitmap;
            // The lpas in trace_lpas_sorted_histogram are those that are
            // actually accessed by the application
            if (lpa <= max_lpa_within_device) {
              if (!is_hot_address && hot_region_last_index_in_histogram == 0) {
                hot_region_last_index_in_histogram =
                    (unsigned int)(trace_lpas_sorted_histogram.size());
              }
              std::pair<int, LPA_type> entry((is_hot_address ? 1000 : 1), lpa);
              trace_lpas_sorted_histogram.insert(entry);
            }
          } else {
            lpa_set_for_preconditioning[lpa] =
                access_status_bitmap | lpa_set_for_preconditioning[lpa];
          }
        }
      }
    } else {
      ...
      for (auto itr = stat->Write_read_shared_addresses.begin();
           itr != stat->Write_read_shared_addresses.end(); itr++) {
        LPA_type lpa = (*itr);
        if (lpa_set_for_preconditioning.size() <
            no_of_logical_pages_in_steadystate) {
          lpa_set_for_preconditioning[lpa] =
              stat->Write_address_access_pattern[lpa].Accessed_sub_units |
              stat->Read_address_access_pattern[lpa].Accessed_sub_units;
        } else {
          break;
        }
      }

      for (auto itr = stat->Read_address_access_pattern.begin();
           itr != stat->Read_address_access_pattern.end(); itr++) {
      ...
      for (auto itr = stat->Write_address_access_pattern.begin();
           itr != stat->Write_address_access_pattern.end(); itr++) {
        LPA_type lpa = (*itr).first;
        if (lpa_set_for_preconditioning.size() <
            no_of_logical_pages_in_steadystate) {
          if (lpa_set_for_preconditioning.find(lpa) ==
      ...
      stat->Address_distribution_type = Utils::Address_Distribution_Type::
          RANDOM_HOTCOLD; // Initially assume that the trace has hot/cold access
                          // pattern
      ...
      // assumes the workload has a uniform access pattern
      if (stat->Write_address_access_pattern.size() >
        ...
        double prev_r = 0.0;
        for (auto itr = trace_lpas_sorted_histogram.begin();
             itr != trace_lpas_sorted_histogram.end(); itr++) {
          hot_region_last_index_in_histogram++;
          hot_region_write_count += (*itr).first;

          f_temp = double(hot_region_last_index_in_histogram) /
                   double(trace_lpas_sorted_histogram.size());
          r_temp = double(hot_region_write_count) /
                   double(stat->Total_accessed_lbas);
        ...
        if ((r_temp > MIN_HOT_REGION_TRAFFIC_RATIO) &&
            ((r_temp / f_temp) > HOT_REGION_METRIC)) {
      ...
      // Step 1-4: If both read and write LPAs are not enough for
      // preconditioning flash storage space, then fill the remaining space
      while (lpa_set_for_preconditioning.size() <
             no_of_logical_pages_in_steadystate) {
        ...
        while (handled_sectors_count < size) {
          ...
          transaction_size =
              page_size_in_sectors -
              (unsigned int)(internal_lsa % page_size_in_sectors);
          if (handled_sectors_count + transaction_size >= size) {
            transaction_size = size - handled_sectors_count;
          }

          LPA_type lpa =
              Convert_host_logical_address_to_device_address(internal_lsa);
          page_status_type access_status_bitmap =
              Find_NVM_subunit_access_bitmap(internal_lsa);
    ...
    std::vector<double>
        steadystate_block_status_probability; // The probability distribution
                                              // function of the number of valid
                                              // pages in a block in the
                                              // steadystate
    double rho =
        stat->Initial_occupancy_ratio * (1 - over_provisioning_ratio) /
        (1 -
         double(GC_and_WL_Unit->Get_minimum_number_of_free_pages_before_GC()) /
             block_no_per_plane);
    switch (decision_dist_type) {
    case Utils::Address_Distribution_Type::
        RANDOM_HOTCOLD: // Estimate the steady-state of the hot/cold traffic
                        // based on the steady-state of the uniform traffic
    {
      double r_to_f_ratio =
          std::sqrt(double(stat->Ratio_of_traffic_accessing_hot_region) /
                    double(stat->Ratio_of_hot_addresses_to_whole_working_set));
      switch (GC_and_WL_Unit->Get_gc_policy()) {
      case GC_Block_Selection_Policy_Type::
          GREEDY: // Based on: B. Van Houdt, "A mean field model for a class of
                  // garbage collection algorithms in flash-based solid state
                  // drives", SIGMETRICS 2013.
      case GC_Block_Selection_Policy_Type::
          FIFO: // Could be estimated with greedy for large page_no_per_block
                // values, as mentioned in //Based on: B. Van Houdt, "A mean
                // field model for a class of garbage collection algorithms in
                // flash-based solid state drives", SIGMETRICS 2013.
      {
        for (unsigned int i = 0; i <= page_no_per_block; i++) {
          steadystate_block_status_probability.push_back(
              Utils::Combination_count(page_no_per_block, i) *
              std::pow(rho, i) * std::pow(1 - rho, page_no_per_block - i));
        }
        Utils::Euler_estimation(
            steadystate_block_status_probability, page_no_per_block, rho, 30,
            0.001, 0.0000000001,
            10000); // As specified in the SIGMETRICS 2013 paper, a larger value
                    // for d-choices (the name of RGA in Van Houdt's paper) will
                    // lead to results close to greedy. We use d=30 to estimate
                    // steady-state of the greedy policy with that of d-chioces.
        break;
      }
      case GC_Block_Selection_Policy_Type::
          RGA: // Based on: B. Van Houdt, "A mean field model for a class of
               // garbage collection algorithms in flash-based solid state
               // drives", SIGMETRICS 2013.
      {
        for (unsigned int i = 0; i <= page_no_per_block; i++) {
        ...
        Utils::Euler_estimation(
            steadystate_block_status_probability, page_no_per_block, rho,
            GC_and_WL_Unit->Get_GC_policy_specific_parameter(), 0.001,
            0.0000000001, 10000);
        break;
      }
      case GC_Block_Selection_Policy_Type::RANDOM:
      case GC_Block_Selection_Policy_Type::
          RANDOM_P: // Based on: B. Van Houdt, "A mean field model for a class
                    // of garbage collection algorithms in flash-based solid
                    // state drives", SIGMETRICS 2013.
      {
        for (unsigned int i = 0; i <= page_no_per_block; i++) {
          steadystate_block_status_probability.push_back(
              rho / (rho + std::pow(1 - rho, i)));
          for (unsigned int j = i + 1; j <= page_no_per_block; j++) {
            steadystate_block_status_probability[i] *=
                ((1 - rho) * j) / (rho + (1 - rho) * j);
          }
        }
        for (int i = page_no_per_block; i > 0; i--) {
          steadystate_block_status_probability[i] =
              steadystate_block_status_probability[i] -
              steadystate_block_status_probability[i - 1];
        }
        break;
      }
      case GC_Block_Selection_Policy_Type::
          RANDOM_PP: // Based on: B. Van Houdt, "A mean field model for a class
                     // of garbage collection algorithms in flash-based solid
                     // state drives", SIGMETRICS 2013.
      {
        ...
        double S_rho_b = 0;
        for (unsigned int j =
                 GC_and_WL_Unit->Get_GC_policy_specific_parameter() + 1;
             j <= page_no_per_block; j++) {
          S_rho_b += 1.0 / double(j);
        }
        double a_r = page_no_per_block -
                     GC_and_WL_Unit->Get_GC_policy_specific_parameter() -
                     page_no_per_block * S_rho_b;
        double b_r = rho * S_rho_b + 1 - rho;
        double c_r = -1 * rho / page_no_per_block;
        double mu_b = (-1 * b_r + std::sqrt(b_r * b_r - 4 * a_r * c_r)) /
                      (2 * a_r); // assume always rho < 1 - 1/b
        for (int i = page_no_per_block; i >= 0; i--) {
            ...
            steadystate_block_status_probability[i] =
                ...
                (i +
                 (rho /
                  (1 - rho -
                   mu_b *
                       (page_no_per_block * S_rho_b - page_no_per_block +
                        GC_and_WL_Unit->Get_GC_policy_specific_parameter()))));
          } else if (i < int(page_no_per_block)) {
            steadystate_block_status_probability[i] =
                double(page_no_per_block * mu_b) / double(i);
          } else {
      ...
      for (int i = 0; i < int(page_no_per_block) + 1; i++) {
        steadystate_block_status_probability_temp.push_back(
            steadystate_block_status_probability[i]);
        average_page_no_per_block +=
            steadystate_block_status_probability[i] * i;
      }

      for (int i = 0; i < int(page_no_per_block) + 1; i++) {
        int phi_index =
            int((double(i) - average_page_no_per_block) * r_to_f_ratio);
        if (phi_index < 0 || phi_index >= int(page_no_per_block)) {
          steadystate_block_status_probability[i] = 0;
        } else {
          steadystate_block_status_probability[i] =
              r_to_f_ratio *
              steadystate_block_status_probability_temp[phi_index];
        }
      }
      double sum = 0;
      for (unsigned int i = 0; i <= page_no_per_block; i++) {
    ...
    case Utils::Address_Distribution_Type::
        STREAMING: // A simple estimation based on: "Stochastic Modeling of
                   // Large-Scale Solid-State Storage Systems: Analysis, Design
                   // Tradeoffs and Optimization", SIGMETRICS 2013
      for (unsigned int i = 0; i <= page_no_per_block; i++) {
        if (i == 0) {
    ...
    default:
      PRINT_ERROR("Unhandled address distribution type in FTL's "
                  "preconditioning function.")
    }

    // Step 3: Distribute LPAs over the entire flash space
    // MQSim assigns PPAs to LPAs based on the estimated steadystate status of
    ...
    if (sum > 1.001 || sum < 0.99) {
      PRINT_ERROR("Wrong probability distribution function for the number of "
                  "valid pages in flash blocks in the steady-state! It is not "
                  "safe to continue preconditioning!")
    }
    Address_Mapping_Unit->Allocate_address_for_preconditioning(
        stat->Stream_id, lpa_set_for_preconditioning,
        steadystate_block_status_probability);

    // Step 4: Touch the LPAs and bring them to CMT to warmup address mapping
    // unit
    if (!Address_Mapping_Unit->Is_ideal_mapping_table()) {
      // Step 4-1: Determine how much share of the entire CMT should be filled
      // based on the flow arrival rate and access pattern
      ...
      case CMT_Sharing_Mode::SHARED: {
        ...
        if (stat->Type == Utils::Workload_Type::SYNTHETIC) {
          ...
          case Utils::Request_Generator_Type::QUEUE_DEPTH: {
            ...
            double avg_arrival_time =
                double(max_arrival_time) / double(stat->Request_queue_depth);
            flow_rate = 1.0 / avg_arrival_time * SIM_TIME_TO_SECONDS_COEFF *
                        stat->Average_request_size_sector;
            break;
          }
          default:
            PRINT_ERROR("Unknown request type generator in the FTL "
                        "preconditioning function.")
          }
        } else {
          flow_rate = 1.0 / double(stat->Average_inter_arrival_time_nano_sec) *
                      SIM_TIME_TO_SECONDS_COEFF *
                      stat->Average_request_size_sector;
        }

        no_of_entries_in_cmt =
            (unsigned int)(double(flow_rate) / double(overall_rate) *
                           Address_Mapping_Unit->Get_cmt_capacity());
        break;
      }
      case CMT_Sharing_Mode::EQUAL_SIZE_PARTITIONING:
        no_of_entries_in_cmt =
            (unsigned int)(1.0 / double(workload_stats.size()) *
                           Address_Mapping_Unit->Get_cmt_capacity());
        ...
        break;
      default:
        PRINT_ERROR("Unknown mapping table sharing mode in the FTL "
                    "preconditioning function.")
      }

      if (max_LPA - min_LPA + 1 <
          LPA_type(trace_lpas_sorted_histogram.size())) {
        no_of_entries_in_cmt =
            (unsigned int)(trace_lpas_sorted_histogram.size());
      }
      ...
      case Utils::Address_Distribution_Type::RANDOM_HOTCOLD: {
        ...
        unsigned int required_no_of_hot_cmt_entries =
            (unsigned int)(stat->Ratio_of_hot_addresses_to_whole_working_set *
                           no_of_entries_in_cmt);
        unsigned int entries_to_bring_into_cmt = required_no_of_hot_cmt_entries;
        if (required_no_of_hot_cmt_entries >
            hot_region_last_index_in_histogram) {
          entries_to_bring_into_cmt = hot_region_last_index_in_histogram;
        }
        auto itr = trace_lpas_sorted_histogram.begin();
        while (Address_Mapping_Unit->Get_current_cmt_occupancy_for_stream(
                   stat->Stream_id) < entries_to_bring_into_cmt) {
          Address_Mapping_Unit->Bring_to_CMT_for_preconditioning(
              stat->Stream_id, (*itr).second);
          trace_lpas_sorted_histogram.erase(itr++);
        }

        // If there is free space in the CMT, then bring the remaining addresses
        // to CMT
        ...
        while (Address_Mapping_Unit->Get_current_cmt_occupancy_for_stream(
                   stat->Stream_id) < no_of_entries_in_cmt) {
          Address_Mapping_Unit->Bring_to_CMT_for_preconditioning(
              stat->Stream_id, (*itr2++).second);
      ...
      case Utils::Address_Distribution_Type::STREAMING: {
        LPA_type lpa;
        LPA_type first_lpa_streaming =
            Convert_host_logical_address_to_device_address(
                stat->First_Accessed_Address);
      ...
      case Utils::Address_Distribution_Type::RANDOM_UNIFORM: {
        int random_walker = int(random_generator.Uniform(
            0, uint32_t(trace_lpas_sorted_histogram.size()) - 2));
        int random_step = random_generator.Uniform_uint(
            0, (uint32_t)(trace_lpas_sorted_histogram.size()) /
                   no_of_entries_in_cmt);
        auto itr = trace_lpas_sorted_histogram.begin();
        while (Address_Mapping_Unit->Get_current_cmt_occupancy_for_stream(
                   stat->Stream_id) < no_of_entries_in_cmt) {
          ...
          if (trace_lpas_sorted_histogram.size() > 1) {
            trace_lpas_sorted_histogram.erase(itr++);
            if (random_walker + random_step >=
                    int(trace_lpas_sorted_histogram.size() - 1) ||
                random_walker + random_step < 0) {
              random_step *= -1;
            }
...
// void FTL::Report_results_in_XML(std::string name_prefix,
...
//   val = std::to_string(double(Stats::Total_page_movements_for_gc) /
//                        double(Stats::Total_gc_executions));
//   xmlwriter.Write_attribute_string_inline(attr, val);
//
...
//   val = std::to_string(double(Stats::Total_page_movements_for_wl) /
//                        double(Stats::Total_wl_executions));
//   xmlwriter.Write_attribute_string_inline(attr, val);
//
//   xmlwriter.Write_end_element_tag();
// }

void FTL::reportResults(fmt::ostream &output) {
  auto header = "flash_read_cmd "
                "flash_interleaved_read_cmd "
                "flash_multiplane_read_cmd "
                "flash_copyback_read_cmd "
                "flash_multiplane_copyback_read_cmd "
                "flash_program_cmd "
                "flash_interleaved_program_cmd "
                "flash_multiplane_program_cmd "
                "flash_interleaved_multiplane_program_cmd "
                "flash_copyback_program_cmd "
                "flash_multiplane_copyback_program_cmd "
                "flash_erase_cmd "
                "flash_interleaved_erase_cmd "
                "flash_multiplane_erase_cmd "
                "flash_interleaved_multiplane_erase_cmd "
                "flash_suspend_program_cmd "
                "flash_suspend_erase_cmd "
                "flash_read_mapping_cmd "
                "flash_program_mapping_cmd "
                "cmt_hits "
                "cmt_hits_for_read "
                "cmt_hits_for_write "
                "cmt_misses "
...
auto fmt::formatter<SSD_Components::FTL>::format(const SSD_Components::FTL &ftl,
  ...
  return fmt::format_to(
      ...
      SSD_Components::Stats::IssuedInterleaveReadCMD,
      SSD_Components::Stats::IssuedMultiplaneReadCMD,
      SSD_Components::Stats::IssuedCopybackReadCMD,
      SSD_Components::Stats::IssuedMultiplaneCopybackReadCMD,
      SSD_Components::Stats::IssuedProgramCMD,
      SSD_Components::Stats::IssuedInterleaveProgramCMD,
      SSD_Components::Stats::IssuedMultiplaneProgramCMD,
      SSD_Components::Stats::IssuedInterleaveMultiplaneProgramCMD,
      SSD_Components::Stats::IssuedCopybackProgramCMD,
      SSD_Components::Stats::IssuedMultiplaneCopybackProgramCMD,
      SSD_Components::Stats::IssuedEraseCMD,
      SSD_Components::Stats::IssuedInterleaveEraseCMD,
      SSD_Components::Stats::IssuedMultiplaneEraseCMD,
      SSD_Components::Stats::IssuedInterleaveMultiplaneEraseCMD,
      SSD_Components::Stats::IssuedSuspendProgramCMD,
      SSD_Components::Stats::IssuedSuspendEraseCMD,
      SSD_Components::Stats::Total_flash_reads_for_mapping,
      SSD_Components::Stats::Total_flash_writes_for_mapping,
      SSD_Components::Stats::CMT_hits,
      SSD_Components::Stats::readTR_CMT_hits,
      SSD_Components::Stats::writeTR_CMT_hits, 
      SSD_Components::Stats::CMT_miss,
      SSD_Components::Stats::readTR_CMT_miss,
      SSD_Components::Stats::writeTR_CMT_miss,
      SSD_Components::Stats::total_CMT_queries,
