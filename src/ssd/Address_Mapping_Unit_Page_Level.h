...
struct CMTSlotType {
  ...
  std::list<std::pair<LPA_type, CMTSlotType *>>::iterator
      listPtr; // used for fast implementation of LRU
...
public:
  ...
  void Update_mapping_info(const stream_id_type streamID, const LPA_type lpa,
                           const PPA_type ppa,
                           const page_status_type pageWriteState);
  void Insert_new_mapping_info(const stream_id_type streamID,
                               const LPA_type lpa, const PPA_type ppa,
                               const unsigned long long pageWriteState);
  page_status_type
  Get_bitmap_vector_of_written_sectors(const stream_id_type streamID,
                                       const LPA_type lpa);

  bool Is_slot_reserved_for_lpn_and_waiting(const stream_id_type streamID,
                                            const LPA_type lpa);
...
/* Each stream has its own address mapping domain. It helps isolation of GC
 * interference (e.g., multi-streamed SSD HotStorage 2014, and OPS isolation in
 * FAST 2015) However, CMT is shared among concurrent streams in two ways: 1)
 * each address mapping domain shares the whole CMT space with other domains,
 * and 2) each address mapping domain has its own share of CMT (equal
...
public:
  AddressMappingDomain(unsigned int cmt_capacity, unsigned int cmt_entry_size,
                       ...
                       unsigned int channel_no, flash_chip_ID_type *chip_ids,
                       unsigned int chip_no, flash_die_ID_type *die_ids,
                       unsigned int die_no, flash_plane_ID_type *plane_ids,
  ...
  void Update_mapping_info(const bool ideal_mapping,
                           const stream_id_type stream_id, const LPA_type lpa,
                           const PPA_type ppa,
                           const page_status_type page_status_bitmap);
  page_status_type Get_page_status(const bool ideal_mapping,
                                   const stream_id_type stream_id,
                                   const LPA_type lpa);
  PPA_type Get_ppa(const bool ideal_mapping, const stream_id_type stream_id,
                   const LPA_type lpa);
  PPA_type Get_ppa_for_preconditioning(const stream_id_type stream_id,
                                       const LPA_type lpa);
  bool Mapping_entry_accessible(const bool ideal_mapping,
                                const stream_id_type stream_id,
                                const LPA_type lpa);

  std::multimap<LPA_type, NVM_Transaction_Flash *>
      Waiting_unmapped_read_transactions;
  std::multimap<LPA_type, NVM_Transaction_Flash *>
  ...
  std::set<LPA_type>
      Locked_LPAs; // Used to manage race conditions, i.e. a user request
                   // accesses and LPA while GC is moving that LPA
  std::set<MVPN_type> Locked_MVPNs; // Used to manage race conditions
  std::multimap<LPA_type, NVM_Transaction_Flash *>
      Read_transactions_behind_LPA_barrier;
  std::multimap<LPA_type, NVM_Transaction_Flash *>
      Write_transactions_behind_LPA_barrier;
...
public:
  Address_Mapping_Unit_Page_Level(
      ...
      Flash_Plane_Allocation_Scheme_Type PlaneAllocationScheme,
      unsigned int ConcurrentStreamNo, unsigned int ChannelCount,
  ...
  void Allocate_address_for_preconditioning(
      const stream_id_type stream_id,
  ...
  void Translate_lpa_to_ppa_and_dispatch(
      const std::list<NVM_Transaction *> &transactionList);
  void Get_data_mapping_info_for_gc(const stream_id_type stream_id,
                                    const LPA_type lpa, PPA_type &ppa,
                                    page_status_type &page_state);
  void Get_translation_mapping_info_for_gc(const stream_id_type stream_id,
                                           const MVPN_type mvpn,
                                           MPPN_type &mppa,
                                           sim_time_type &timestamp);
  void Allocate_new_page_for_gc(NVM_Transaction_Flash_WR *transaction,
                                bool is_translation_page);
  ...
  void Convert_ppa_to_address(const PPA_type ppn,
                              NVM::FlashMemory::Physical_Page_Address &address);
  PPA_type Convert_address_to_ppa(
      const NVM::FlashMemory::Physical_Page_Address &pageAddress);

  void Set_barrier_for_accessing_physical_block(
...
private:
  ...
  unsigned int CMT_entry_size,
      GTD_entry_size; // In CMT MQSim stores (lpn, ppn, page status bits) but in
                      // GTD it only stores (ppn, page status bits)
  ...
  void allocate_page_in_plane_for_translation_write(
      NVM_Transaction_Flash *transaction, MVPN_type mvpn, bool is_for_gc);
  void allocate_plane_for_preconditioning(
  ...
  bool translate_lpa_to_ppa(stream_id_type streamID,
                            NVM_Transaction_Flash *transaction);
  std::set<NVM_Transaction_Flash_WR *> ***
      *Write_transactions_for_overfull_planes;

  void
  generate_flash_read_request_for_mapping_data(const stream_id_type streamID,
                                               const LPA_type lpn);
  void generate_flash_writeback_request_for_mapping_data(
