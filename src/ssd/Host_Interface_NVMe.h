...
public:
  Input_Stream_NVMe(IO_Flow_Priority_Class::Priority priority_class,
      ...
      : Input_Stream_Base(), Priority_class(priority_class),
        ...
        Completion_queue_size(completion_queue_size), Submission_head(0),
        Submission_head_informed_to_host(0), Submission_tail(0),
  ...
  uint16_t
      Submission_head_informed_to_host; // To avoide race condition, the
                                        // submission head must not be informed
                                        // to host until the request info of the
                                        // head is successfully arrived
  uint16_t Submission_tail;
  uint16_t Completion_head;
  uint16_t Completion_tail;
  std::list<User_Request *>
      Waiting_user_requests; // The list of requests that have been fetch to the
                             // device queue and are getting serviced
  std::list<User_Request *>
      Completed_user_requests; // The list of requests that are completed but
                               // have not been informed to the host due to full
                               // CQ
  std::list<User_Request *>
      Waiting_write_data_transfers; // The list of write requests that are
                                    // waiting for data
  uint16_t
      On_the_fly_requests; // the number of requests that are either being fetch
                           // from host or waiting in the device queue
};

class Input_Stream_Manager_NVMe : public Input_Stream_Manager_Base {
public:
  Input_Stream_Manager_NVMe(Host_Interface_Base *host_interface,
                            uint16_t queue_fetch_szie);
  unsigned int Queue_fetch_size;
  stream_id_type Create_new_stream(
      IO_Flow_Priority_Class::Priority priority_class,
      LHA_type start_logical_sector_address,
      LHA_type end_logical_sector_address,
  ...
  void Submission_queue_tail_pointer_update(stream_id_type stream_id,
                                            uint16_t tail_pointer_value);
  void Completion_queue_head_pointer_update(stream_id_type stream_id,
...
private:
  ...
  void inform_host_request_completed(stream_id_type stream_id,
...
public:
  ...
  void Send_completion_queue_element(User_Request *request,
                                     uint16_t sq_head_value);
...
public:
  Host_Interface_NVMe(const sim_object_id_type &id,
                      ...
                      uint16_t queue_fetch_size, unsigned int sectors_per_page,
                      Data_Cache_Manager_Base *cache);
  stream_id_type
  Create_new_stream(IO_Flow_Priority_Class::Priority priority_class,
                    LHA_type start_logical_sector_address,
                    LHA_type end_logical_sector_address,
                    uint64_t submission_queue_base_address,
                    uint64_t completion_queue_base_address);
