...
#define DELETE_REQUEST_NVME(REQ)                                               \
  ...
  if (REQ->Transaction_list.size() != 0)                                       \
    PRINT_ERROR("Deleting an unhandled user requests in the host interface! "  \
                "MQSim thinks something is going wrong!")                      \
  delete REQ;
...
public:
  ...
  uint32_t Get_average_write_transaction_waiting_time(
      stream_id_type stream_id); // in microseconds
  uint32_t Get_read_transaction_count(
      stream_id_type stream_id);
  uint32_t Get_write_transaction_count(
      stream_id_type stream_id);
  uint32_t Get_total_transaction_count(
      stream_id_type stream_id);
  void init();
protected:
...
protected:
  ...
  void broadcast_user_request_arrival_signal(User_Request *user_request) {
    for (std::vector<UserRequestArrivedSignalHandlerType>::iterator it =
             connected_user_request_arrived_signal_handlers.begin();
  ...
  static void
  handle_user_request_serviced_signal_from_cache(User_Request *user_request) {
